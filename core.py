...
##assess the output flags from the detrending
    ...
    # old version -- seems to work the best with fstep = .00002 or .00001, smaller seems to be worse?
    fstep = 0.00002
...
#     # new version
    ttotal = max(data.t) - min(data.t)
    startperiod = searchmin # 2 hours in days
    density = 0.28 # can increase
    pmax = searchmax
    ...
    while pgrid[-1] < pmax:
        dgrid.append((pgrid[-1] * (365.25 ** 2) / ((np.pi ** 3) * density * (215.0 ** 3))) ** (1 / 3))  
        pgrid.append(pgrid[-1] + (pgrid[-1] * dgrid[-1] / ttotal) / 3)

    
    dgrid, pgrid = np.asarray(dgrid), np.asarray(pgrid)     
    ...
    dp     = np.zeros(howManyKeep)
    best_p = np.zeros(howManyKeep)
    detsig = np.zeros(howManyKeep)
    t0     = np.zeros(howManyKeep)
    dcyc   = np.zeros(howManyKeep)


    for cnt in range(howManyKeep):
        ...
        ## new astropy bls
        unc = median_absolute_deviation(detrend[good[torun]])
        model = BoxLeastSquares(data.t[good[torun]] * units.day, detrend[good[torun]], dy=unc)
        periodogram = model.power(pgrid,np.arange(dgrid[1],dgrid[-1], .01))
        power = periodogram.power
        best_pow = np.max(power[np.where(pgrid > searchmin)])
        thisbest_p = pgrid[np.where(power == best_pow)]
